...
The aggregation to create the global training model is framework-agnostic, while the collaboration from local training models using local datasets supports any deep learning frameworks, such as `TensorFlow\ :sup:`*` \ <https://www.tensorflow.org/>`_ or `PyTorch\ :sup:`*` \ <https://pytorch.org/>`_, via a plugin framework.

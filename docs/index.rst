...
The aggregation to create the global training model is framework-agnostic, while the collaboration from local training models using local datasets supports any deep learning frameworks, such as `TensorFlow\ :sub:`*`\ <https://www.tensorflow.org/>`_ or `PyTorch\ :sub:`*`\ <https://pytorch.org/>`_, via a plugin framework.

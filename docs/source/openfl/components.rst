...
Converting tensors is done by Framework adapter plugins. |productName| ships with Pytorch and Tensorflow 2.x framework adapters. 
...
The adapter plugin interface is simple: there are two required methods to load and extract tensors from a model and optimizer. 

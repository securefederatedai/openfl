...
def FedAvg(models, previous_global_model=None, dp_params=None):
    ...
    if dp_params is not None and previous_global_model is not None:
        ...
        for non_delta_state in non_delta_states:
            delta_states.append({key: non_delta_state[key] - previous_global_model_state[key] for key in non_delta_state})
...
def add_noise_on_aggegated_parameters(collaborators, model, dp_params):
    ...
    normal_distribution = Normal(loc=0, scale=dp_params['noise_multiplier'] * dp_params['clip_norm'])
    with torch.no_grad():
        for param_tensor in model.state_dict():
            noise_samples = normal_distribution.sample(state_dict[param_tensor].shape) / int(dp_params['sample_rate'] * float(len(collaborators)))
...
class FederatedFlow(FLSpec):
    ...
    def start(self):
        ...
        if self.dp_params is None:
            ...
            global_data_loader = DataLoader(self.collaborators, batch_size=int(self.sample_rate * float(len(self.collaborators))))
        ...
        if len(self.round_collaborators) !=0:
            if not isinstance(self.round_collaborators[0], torch.Tensor):
                ...
                print(f"Collaborators patricipated in Round: {self.round}", self.round_collaborators)
                print(20*"#") 
    ...
    def train(self):
        ...
        for batch_idx, (data, target) in enumerate(self.train_loader):
            ...
            self.optimizer.step(global_model_state=self.previous_global_model.state_dict(), last_iter= (batch_idx==(len(self.train_loader)-1)))
    ...
    def join(self,inputs):
        ...
        if self.dp_params is not None:
            self.model = add_noise_on_aggegated_parameters(self.collaborators, self.model, self.dp_params)
            self.privacy_accountant.step(noise_multiplier=self.dp_params['noise_multiplier'], sample_rate=self.sample_rate)
            epsilon, best_alpha = self.privacy_accountant.get_privacy_spent(delta=self.dp_params['delta'])
            print(20*"#")

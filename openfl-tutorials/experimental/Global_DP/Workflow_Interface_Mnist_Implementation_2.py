...
class GlobalModelTools(object):
    ...
    def __init__(self, example_model_state, global_model, collaborator_names, dp_params=None):
        self.example_state = example_model_state
        self.global_model = global_model
        self.collaborator_names = collaborator_names
        self.global_optimizer = torch.optim.SGD(params=self.global_model.parameters(), lr=1.0) # This choice of optimizer is required for correct model aggregation 
        ...
        self.global_data_loader = torch.utils.data.DataLoader(TensorDataset(torch.Tensor([idx for idx in range(len(self.collaborator_names))]).to(torch.int)), 
                                                             batch_size=int(sample_rate * float(len(collaborator_names))), 
                                                             shuffle=True)
        if dp_params is not None:
            self.global_model, self.global_optimizer, self.global_data_loader = self.privacy_engine.make_private(module=self.global_model, 
    ...
    def populate_model_params_and_gradients(self, state_for_params, states_for_gradients):
        if self.dp_params is not None:
            ...
            per_layer_grad_samples = {key: [state[key] for state in states_for_gradients] for key in states_for_gradients[0].keys()}
...
def FedAvg(models, global_model_tools, previous_global_state, dp_params):
    ...
    for non_delta_state in non_delta_states:
        ...
        neg_delta_states.append({key: -non_delta_state[key] + previous_global_state[key] for key in non_delta_state})
    ...
    for idx, state in enumerate(neg_delta_states):
        ...
        if delta_norm > dp_params['clip_norm']:
            raise ValueError(f"The model with index {idx} had update whose L2-norm was greater than clip norm. Correct the local periodic clipping.")
    print()
    # Clearing state from optimizer from last round so as to not leak information
    global_model_tools.global_optimizer.zero_grad()
    global_model_tools.populate_model_params_and_gradients(state_for_params=previous_global_state, states_for_gradients=neg_delta_states)
    ...
    new_model.load_state_dict({key[8:]: value for key, value in global_model_tools.global_model.state_dict().items()})
    return new_model
...
def optimizer_to_device(optimizer, device):
    if optimizer.state_dict != {}:
        ...
        raise(ValueError(f'Current optimizer state does not have dict keys: please verify'))

def clip_testing_on_optimizer_parameters(optimizer_before_step_params, optimizer_after_step_params, collaborator_name, round_num, device):
    len_equal_tensor = 0 
    for param_idx in range(len(optimizer_after_step_params)):
        for tensor_1, tensor_2 in zip(optimizer_before_step_params[param_idx], optimizer_after_step_params[param_idx]):
            if torch.equal(tensor_1.to(device), tensor_2.to(device)) is True:
                len_equal_tensor += 1
    if len_equal_tensor == len(optimizer_after_step_params):
        raise (ValueError(f'No clipping effect: Optimizer param data is the same between before and after optimizer step for collaborator: {collaborator_name} at round {round_num}'))

def validate_dp_params(dp_params):
    # The differential privacy block should have the exact keys provided below
    required_dp_keys = ['clip_norm', 'noise_multiplier', 'delta', 'sample_rate', 'clip_frequency']
    ...
    if deficit != []:
        raise ValueError("The 'differential_privacy' block is missing the required keys: {}".format(deficit))
...
class FederatedFlow(FLSpec):
    ...
    def __init__(self, config_path, model, collaborator_names, device, total_rounds=10, top_model_accuracy=0, clip_test=False, **kwargs):
        ...
        self.global_model_tools = GlobalModelTools(global_model=self.global_model,
    ...
    def start(self):
        ...
        round_collaborator_idxs = [batch[0] for batch in self.global_model_tools.global_data_loader][0]
        self.round_collaborators = [self.collaborator_names[idx] for idx in round_collaborator_idxs]
        
        if self.round is None:
            self.round = 0
            if self.dp_params is None:
                self.optimizers = {collaborator_name: default_optimizer(self.model) for collaborator_name in self.collaborator_names}
            else:
                self.optimizers = {collaborator_name: ClipOptimizer(base_optimizer=default_optimizer(self.model),
                                                   device=self.device, 
                                                   clip_norm=self.dp_params['clip_norm'], 
                                                   clip_freq=self.dp_params['clip_frequency']) for collaborator_name in self.collaborator_names}
        ...
        if (len(round_collaborator_idxs) !=0) and (round_collaborator_idxs[0].nelement() != 0): #(not isinstance(round_collaborator_idxs[0], torch.Tensor)):
            self.next(self.aggregated_model_validation, foreach='round_collaborators', exclude=['private'])
        else:
            if (self.round == self.total_rounds-1):
                print(f'Completed all rounds with no collaborator selected for training at any round')
                self.next(self.end)
    ...
    def train(self):
        ...
        self.optimizer = ClipOptimizer(base_optimizer=default_optimizer(self.model),
                                                   device=self.device, 
                                                   clip_norm=self.dp_params['clip_norm'], 
        ...
        for batch_idx, (data, target) in enumerate(self.train_loader):
            ...
            if self.clip_test:
                optimizer_before_step_params = [param.data for param in self.optimizer.param_groups()[0]['params']]

            self.optimizer.step(global_model_state=self.global_model.state_dict(), last_iter= (batch_idx==(len(self.train_loader)-1)))

            if self.dp_params is not None:
                if batch_idx % self.dp_params['clip_frequency'] == 0 or (batch_idx==(len(self.train_loader)-1)):
                    if self.clip_test:
                        optimizer_after_step_params = [param.data for param in self.optimizer.param_groups()[0]['params']]
                        clip_testing_on_optimizer_parameters(optimizer_before_step_params, optimizer_after_step_params, self.collaborator_name, self.round, self.device)
    ...
    def join(self, inputs):
        ...
        self.model.load_state_dict(FedAvg([input.model.cpu() for input in inputs], 
                            global_model_tools=self.global_model_tools, 
                            previous_global_state=self.global_model.cpu().state_dict(), 
                            dp_params=self.dp_params).state_dict()
        )
    ...
    def check_round_completion(self):
        if self.round == self.total_rounds-1:
            ...
            # determine starting round collaborators by grabbing first batch of loader
            round_collaborator_idxs = [batch for batch in self.global_model_tools.global_data_loader][0]
            self.round_collaborators = [self.collaborator_names[idx] for idx in round_collaborator_idxs[0]]
...
if __name__ == '__main__':
    ...
    argparser.add_argument(
        '--config_path',
        help='Absolute path to the flow configuration file.')
    argparser.add_argument(
        '--clip_test',
        default=False, 
    ...
    collaborator_names = ['Portland', 'Seattle', 'Bangalore', 'Chandler', 'Austin', 
                            'Hudson', 'Penang', 'Haifa', 'CostaRica', 'Guadalajara']
    collaborators = [Collaborator(name=name) for name in collaborator_names]
    
    if torch.cuda.is_available():
        device = torch.device(f'cuda:0')  # This will enable Ray library to reserve available GPU(s) for the task
    else:
        device = torch.device(f'cpu')     # Uncomment appropriate collaborator decorators in FederatedFlow class if you want the application ro run on CPU
    ...
    flflow = FederatedFlow(config_path=args.config_path, 
                           model=initial_model, 

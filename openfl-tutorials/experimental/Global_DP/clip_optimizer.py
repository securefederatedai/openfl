...
class ClipOptimizer(object):
    ...
    def step(self, global_model_state, last_iter) -> None:    
        ...
        if self.counter % self.clip_freq == 0 or last_iter:
            ...
            delta_params = [torch.sub(local_params[idx], global_model_state[ordered_state_keys[idx]]) for idx in range(len(local_params))]

            # calculate clip factor
            per_param_delta_norm = torch.stack([torch.norm(param, dim=()) for param in delta_params])
            ...
            # clip delta
            updated_params = [torch.add(global_model_state[ordered_state_keys[idx]], torch.mul(delta_params[idx], clip_factor)) for idx in range(len(delta_params))]

            # clipped model
            for idx, param in enumerate(local_params):
                if param.data.shape != updated_params[idx].shape:
                    raise ValueError(f"Trying to update params of shape: {param.data.shape} with update of shape: {updated_params[idx].shape}")

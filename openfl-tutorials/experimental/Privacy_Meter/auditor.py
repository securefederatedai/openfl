...
def PopulationAuditor(target_model,datasets,pm_info):
    ...
    if torch.is_tensor(train_dataset.data):
        ...
        population_ds = {'x':  torch.from_numpy(population_dataset.data).float(), 'y': torch.tensor(population_dataset.targets)}
    ...
    reference_info_source = InformationSource(
        models=[target_model],
        datasets=[pm_population_dataset]
    ) 
    ...
    for idx in range(len(fpr_tolerance_list)):
        for key in history_dict:
            for sidx,signal in enumerate(history_dict[key].signals):
                ...
                plt.plot(history_dict[key].history['round'], hist,label=f'{key}-{signal}')
        plt.title('{} (FPR @ {})'.format(client,fpr_tolerance_list[idx]))
        plt.legend()
        plt.savefig('{}/{}_tpr_at_{}.png'.format(history_dict[key].log_dir,client,fpr_tolerance_list[idx]),dpi=200)
        plt.clf()
...
def plot_auc_history(history_dict, client):
    ...
    for key in history_dict:
        for sidx,signal in enumerate(history_dict[key].signals):
            plt.plot(history_dict[key].history['round'], np.array(history_dict[key].history['auc'])[:,sidx],label=f'{key}-{signal}')
...
def plot_roc_history(history_dict, client):
    ...
    plt.legend()
    plt.savefig('{}/{}_roc_at_{}.png'.format(history_dict[key].log_dir,client,round_num),dpi=200)
    plt.clf()

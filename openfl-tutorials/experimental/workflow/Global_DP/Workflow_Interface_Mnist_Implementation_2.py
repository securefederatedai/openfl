...
class GlobalModelTools(object):
    ...
    def __getstate__(self):
        ...
        # Remove the optimizer and privacy engine from the state
        state['global_optimizer'] = None
        state['privacy_engine'] = None
        return state

    def __setstate__(self, state):
        self.__dict__.update(state)
        # Recreate the optimizer and privacy engine
        self.global_optimizer = torch.optim.SGD(
            params=self.global_model.parameters(), lr=1.0
        )
        self.privacy_engine = PrivacyEngine()
        if self.dp_params is not None:
            (
                ...
                max_grad_norm=self.dp_params["clip_norm"],
            )
...
if __name__ == "__main__":
    ...
    def callable_to_initialize_aggregator_private_attributes():
        ...
        return {'global_model_tools': GlobalModelTools(

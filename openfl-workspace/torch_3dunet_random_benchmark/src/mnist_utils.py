...
def _load_raw_datashards(shard_num, collaborator_count, transform=None):
    ...
    X_train = np.array([torch.ones(()).new_empty((1, 80, 80, 80)).numpy() for _ in range(100)])
    y_train = np.array([torch.ones(()).new_empty((1, 80, 80, 80)).numpy() for _ in range(100)])

    X_valid = np.array([torch.ones(()).new_empty((1, 80, 80, 80)).numpy() for _ in range(100)])
    y_valid = np.array([torch.ones(()).new_empty((1, 80, 80, 80)).numpy() for _ in range(100)])

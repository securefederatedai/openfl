    ...
    if type(tensors[0]) in (dict, collections.OrderedDict):
        ...
        for key in input_state_dict_keys:
            if (key[-2:] == '_u' or key[-2:] == '_v' or key[-2:] == '_w'):
                tmp_state_dict[key] = np.sum([tensor[key].detach().cpu()
                                             if type(tensor[key]) is torch.Tensor
                                             else tensor[key].cpu() for tensor in tensors], axis=0)
                continue
            tmp_state_dict[key] = np.average([tensor[key].detach().cpu()
                                             if type(tensor[key]) is torch.Tensor
                                             else tensor[key].cpu() for tensor in tensors],

...
from openfl.experimental.workflow.transport.grpc.director_client import EnvoyClient
...
class Envoy:
    ...
    Attributes:
        ...
        envoy_client (EnvoyClient): The envoy client.
    ...
    def __init__(
        ...
        *,
        ...
        self.envoy_client = self._create_envoy_client(director_host, director_port)
    ...
    def _create_envoy_client(self, director_host: str, director_port: int) -> EnvoyClient:
        """Create a EnvoyClient instance.
        ...
        Returns:
            EnvoyClient: Instance of the client
        """
        return EnvoyClient(
    ...
    def _run(self) -> None:
        ...
        while True:
            try:
                ...
                experiment_name = self.envoy_client.wait_experiment()
                data_stream = self.envoy_client.get_experiment_data(experiment_name)
    ...
    def _send_health_check(self) -> None:
        ...
        while True:
            try:
                timeout = self.envoy_client.send_health_check(
                    envoy_name=self.name,
                ...
                self.envoy_client.connect_envoy(envoy_name=self.name)
            time.sleep(timeout)
    ...
    def start(self) -> None:
        """Start the envoy"""
        try:
            is_accepted = self.envoy_client.connect_envoy(envoy_name=self.name)
        except Exception as exc:

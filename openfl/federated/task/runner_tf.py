...
class TensorFlowTaskRunner(TaskRunner):
    ...
    def train_batches(self,
        ...
        tags = ('trained', )
        output_metric_dict = {
            TensorKey(self.loss_name, origin, round_num, True, ('metric', )):
    ...
    def validate(self,
        ...
        tags = ('metric', suffix)
        output_tensor_dict = {
            TensorKey(self.validation_metric_name, origin, round_num, True, tags):
            np.array(score)
        }
    ...
    def validate_batch(self, X, y):
        ...
        return self.sess.run([self.output, self.validation_metric],
                             feed_dict=feed_dict)
    ...
    def set_tensor_dict(self, tensor_dict, with_opt_vars):
        ...
        if with_opt_vars:
            self.assign_ops, self.placeholders = tf_set_tensor_dict(
                tensor_dict, self.sess, self.fl_vars, self.assign_ops,
                self.placeholders)
        else:
            self.tvar_assign_ops, self.tvar_placeholders = tf_set_tensor_dict(
                tensor_dict, self.sess, self.tvars, self.tvar_assign_ops,
...
def tf_set_tensor_dict(tensor_dict,
                       session,
                       variables,
                       assign_ops=None,
                       placeholders=None):
    ...
    if placeholders is None:
        placeholders = {
            v.name: tf.placeholder(v.dtype, shape=v.shape)
            for v in variables
        }
    if assign_ops is None:
        assign_ops = {
            v.name: tf.assign(v, placeholders[v.name])
            for v in variables
        }

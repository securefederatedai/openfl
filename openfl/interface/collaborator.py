...
def generate_cert_request(collaborator_name, data_path, silent, skip_package):
    ...
    (CA_PKI_DIR / 'client').mkdir(parents=True, exist_ok=True)

    echo('  Moving COLLABORATOR certificate to: ' + style(
        f'{CA_PKI_DIR}/{file_name}', fg='green'))

    # Write collaborator csr and key to disk
    write_crt(client_csr, CA_PKI_DIR / 'client' / f'{file_name}.csr')
    write_key(client_private_key, CA_PKI_DIR / 'client' / f'{file_name}.key')
    ...
    if not skip_package:
        ...
        copytree(f'{CA_PKI_DIR}/client', tmp_dir, ignore=ignore)
...
def certify(collaborator_name, silent, request_pkg=False, import_=False):
    ...
    if not import_:
        if request_pkg:
            Path(f'{CA_PKI_DIR}/client').mkdir(parents=True, exist_ok=True)
            unpack_archive(request_pkg, extract_dir=f'{CA_PKI_DIR}/client')
            csr = glob(f'{CA_PKI_DIR}/client/*.csr')[0]
        else:
            if collaborator_name is None:
                ...
                return
            csr = glob(f'{CA_PKI_DIR}/client/col_{common_name}.csr')[0]
            copy(csr, CA_PKI_DIR)
        ...
        if not Path(CA_PKI_DIR / signing_key_path).exists():
            echo(style('Signing key not found.', fg='red')
                 + ' Please run `fx workspace certify`'
                   ' to initialize the local certificate authority.')

        signing_key = read_key(CA_PKI_DIR / signing_key_path)

        # Load signing cert
        if not Path(CA_PKI_DIR / signing_crt_path).exists():
            echo(style('Signing certificate not found.', fg='red')
                 + ' Please run `fx workspace certify`'
                   ' to initialize the local certificate authority.')

        signing_crt = read_crt(CA_PKI_DIR / signing_crt_path)
        ...
        if silent:
            ...
            register_collaborator(CA_PKI_DIR / 'client' / f'{file_name}.crt')
        ...
        copy(f'{CA_PKI_DIR}/client/{file_name}.crt', f'{tmp_dir}/client/')
        # Copy the CA certificate chain to the temporary directory
        copy(f'{CA_PKI_DIR}/cert_chain.crt', tmp_dir)
        ...
        previous_crts = glob(f'{CA_PKI_DIR}/client/*.crt')
        unpack_archive(import_, extract_dir=CA_PKI_DIR)
        updated_crts = glob(f'{CA_PKI_DIR}/client/*.crt')

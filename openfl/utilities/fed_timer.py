...
class CustomThread(Thread):
    ...
    This class extends the `threading.Thread` class and allows for the storage of the 
...
class PrepareTask():
    ...
    Attributes:
        ...
        max_timeout (int): The maximum time to allow for the function's execution.
    ...
    async def async_execute(self):
        """Execute the task asynchronously of the
    ...
    def sync_execute(self):
        """Execute the task synchronously of the
        decorated function referenced by `self._target_fn`.
...
class SyncAsyncTaskDecoFactory:
    ...
    This class is a factory for creating decorators for synchronous and asynchronous tasks.
    Task decorator factory allows creation of concrete implementation of `wrapper` interface 
    and `contextmanager` to setup a common functionality/resources shared by `async_wrapper` 
    and `sync_wrapper`.
    ...
    def __call__(self, func):
        ...
        Call to `@fedtiming()` executes `__call__()` method delegated from the 
        derived class `fedtiming` implementing `SyncAsyncTaskDecoFactory`.
...
class fedtiming(SyncAsyncTaskDecoFactory):  # noqa: N801
    ...
    def wrapper(self, func, *args, **kwargs):
        ...
        This method sets up the task for execution and measures its execution time.
        Yields the control back to `async_wrapper` or `sync_wrapper` function call.
        ...
        Raises:
            Exception: If an error occurs during the function's execution raised by `async_wrapper`

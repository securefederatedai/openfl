...
if __name__ == '__main__':
    ...
    parser.add_argument('--agg-key-path', default=Path.cwd())
    parser.add_argument('--col1-cert-path', default=Path.cwd())
    parser.add_argument('--col1-key-path', default=Path.cwd())
    parser.add_argument('--col2-cert-path', default=Path.cwd())
    parser.add_argument('--col2-key-path', default=Path.cwd())
    ...
    os.makedirs(args.agg_cert_path, exist_ok=True)
    os.makedirs(args.agg_key_path, exist_ok=True)
    os.makedirs(args.col1_cert_path, exist_ok=True)
    os.makedirs(args.col1_key_path, exist_ok=True)
    os.makedirs(args.col2_cert_path, exist_ok=True)
    os.makedirs(args.col2_key_path, exist_ok=True)

    ca_dir_path = Path(args.agg_cert_path).resolve()
    agg_cert_path = Path(args.agg_cert_path).resolve()
    agg_key_path = Path(args.agg_key_path).resolve()
    col1_cert_path = Path(args.col1_cert_path).resolve()
    col1_key_path = Path(args.col1_key_path).resolve()
    col2_cert_path = Path(args.col2_cert_path).resolve()
    col2_key_path = Path(args.col2_key_path).resolve()
    save_model = args.save_model

    # START
    # =====
    # Make sure you are in a Python virtual environment with the FL package installed.
    create_certified_workspace(fed_workspace, template, fqdn,
                               rounds_to_train, ca_dir_path, agg_cert_path, agg_key_path)
    certify_aggregator(fqdn, agg_cert_path, agg_key_path)
    ...
    create_collaborator(col1, workspace_root, col1_data_path, archive_name,
                        fed_workspace, col1_cert_path, col1_key_path, agg_cert_path, agg_key_path)

    # Create collaborator #2
    create_collaborator(col2, workspace_root, col2_data_path, archive_name,
                        fed_workspace, col2_cert_path, col2_key_path, agg_cert_path, agg_key_path)

    # Run the federation
    with ProcessPoolExecutor(max_workers=3) as executor:
        executor.submit(
            check_call, ['fx', 'aggregator', 'start', '-c', agg_cert_path, '-k', agg_key_path],
            cwd=workspace_root)
        time.sleep(5)

        dir1 = workspace_root / col1 / fed_workspace
        executor.submit(
            check_call, ['fx', 'collaborator', 'start', '-n', col1,
                         '-c', col1_cert_path, '-k', col1_key_path],
            cwd=dir1)

        dir2 = workspace_root / col2 / fed_workspace
        executor.submit(
            check_call, ['fx', 'collaborator', 'start', '-n', col2,
                         '-c', col2_cert_path, '-k', col2_key_path],
            cwd=dir2)
